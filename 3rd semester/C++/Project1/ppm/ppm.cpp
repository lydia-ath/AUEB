...
namespace imaging
{
	...
	*/
	...
	float * ReadPPM(const char * filename, int * w, int * h) {
		...
		//seeking to read the file of binary type from where the header ends. Ta diavazo se char giati einai 1 byte xoritikothtas kai emeis diavazoume ta stoixeia ana byte
		float *buffer;  // -16 giati agnoo ta 16 prota bytes pou einai h epikefalida me ta 4 stoixeia
		char* pChars = new  char[3 * (*w)*(*h) - 16];
		ifs.seekg(16, ios::beg);
		ifs.read(pChars, 3*(*w)*(*h));
		ifs.close();
		// me auto ton tropo allazei tin morfi ton stoixeion apo char se float oste na apo8ikeutoun ari8moi 0-255
		buffer = reinterpret_cast<float *>(pChars);
		return buffer;
	...
	*/
	bool WritePPM(const float * data, int w, int h, const char * filename) {
		...
		if (buf == nullptr) return false; //data nullptr
